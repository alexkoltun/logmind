...
.controller('dynamicmenu', function($scope, $routeParams, $http, eventBus, timer) {
  ...
  $scope.generate_sub = function(item) {
      ...
      if (item.type === "container") {
          ...
          ret += "<a href=\"#\"><i class=\"icon " + item.icon + "\"></i> <span>" + item.title + "</span><i class=\"icon-chevron-down\" style=\"float: right; margin-right: 20px;\"></i></a>";
          ret += "<ul>";
          ...
          ret += "<li><a href=\"" + item.link + "\"><i class=\"icon " + item.icon + "\"></i> <span>" + item.title + "</span></a></li>";

      } else if (item.type === "dynamic") {
          if (item.items != undefined) {
              $.each(item.items, function() {
                  ret += "<li><a href=\"" + item.link_base + this[item.link_column] + "\"><i class=\"icon " + item.icon + "\"></i> <span>" + this[item.title_column] + "</span></a></li>";
              });
          }
      }
  ...
  $scope.init = function() {
    ...
    $scope.ejs.Document("logmind-management", "openmind-menu", "default").doGet(function(a) {
        ...
        _menu = angular.fromJson(a._source.menu_json);
